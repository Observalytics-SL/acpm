...
A newly identified individual draft, **draft-aevum-agentcard** (IETF, no WG, no stream, June 2026), is the closest match yet found to ACPM's space. It defines a machine-readable Agent Card with ULID identity, dot-namespaced capabilities, endpoint specifications, and energy-cost pricing. It has no WG adoption, no stream assignment, and is at "I-D Exists" IESG state — an individual submission with no standardisation track. It does not cover trust attestation posture, SLA commitments, delegation rules, or cross-domain applicability (agent/platform/tool/model); ACPM's scope remains broader. This draft should be cited in the comparison table if and when ACPM is submitted to the IETF Datatracker.

Taken together, no found existing standard does what ACPM's core combination does: a single, vendor-neutral, cross-domain (agent/platform/tool/model) profile combining capability support level, trust/attestation, cost, SLA, and delegation rules in one document. The closest individual pieces — draft-aevum-agentcard's capability declarations, HACP's risk tiers, MCP Server Cards' discovery metadata, AgBOM's inventory model — each cover one facet of this narrowly and within a single domain. This section states that plainly rather than overselling: the *pieces* of ACPM's idea exist elsewhere; what is actually new is the combination, and the fact that it is meant to apply uniformly across agents, platforms, tools, and models rather than to one of those at a time.
...
| **draft-aevum-agentcard** (IETF individual I-D, no WG, June 2026) | Machine-readable Agent Card: ULID identity, dot-namespaced capabilities, endpoint specs, energy-cost pricing. | Closest found prior art to ACPM's capability+cost declaration. | Individual I-D, no stream, no WG adoption. Missing: trust attestation posture, SLA, delegation rules, multi-subject-type scope. ACPM is the broader superset; cite this draft in Datatracker submission. |
